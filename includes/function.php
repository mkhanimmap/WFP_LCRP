...
function loadPartner($indicator)
 {
	 $dbconn = pg_connect(CONNSTRCARTODB)
		or die('Could not connect: ' . pg_last_error());
		$query = "select distinct(partner_id) as pid,partner_name from ai_reports_2015_04_14_07_22 where regexp_replace(indicator_name, '&', 'and', 'g') in(".$indicator.") order by partner_name asc";
		$result = pg_query($query) or die('Query failed: ' . pg_last_error());
...
{
 $dbconn = pg_connect(CONNSTRCARTODB);
//$query = "select trim((string_to_array(indicator_name, ' '))[1]) as BeneficiaryGroup from ai_reports_2015_04_14_07_22 where trim((string_to_array(indicator_name, ' '))[1]) not in('#','AffLeb-','AffLeb-#','Amount','Female','Male') and  indicator_name in(".$indicator.")   group by BeneficiaryGroup order by BeneficiaryGroup";
$query = "select trim((string_to_array(indicator_name, ' '))[1]) as BeneficiaryGroup from ai_reports_2015_04_14_07_22 where trim((string_to_array(indicator_name, ' '))[1]) not in('#','AffLeb-','AffLeb-#','Amount') and  regexp_replace(indicator_name, '&', 'and', 'g') in(".$indicator.")   group by BeneficiaryGroup order by BeneficiaryGroup";
